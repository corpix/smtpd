...
func TestGoodParses(t *testing.T) {
	...
	for _, inp := range smtpValidTests {
		s, err = ParseCmd(inp.line)
		if err != nil {
			t.Fatalf("command failed on '%s': error '%s'", inp.line, err.Error())
		}
		if s.Cmd != inp.cmd {
...
var smtpInvalidTests = []struct {
	line string // Input line
	err  error  // Output err to check if non-empty
}{
	{"argble", ErrCmdUnrecognized},
	// UTF-8, and I want to test that this is specifically recognized
	// in an otherwise valid command
	{"MAIL FROM:<Å@fred.com>", ErrCmdContainsNonASCII},

	// prefix validation
	{"VRFYFred", ErrCmdUnrecognized},
	{"MAIL FROMFred", ErrCmdUnrecognized},

	// malformed or missing addresses
	{"MAIL FROM <fred>", ErrCmdImpropperArgFmt},
	{"RCPT TO:  <fred> ", ErrCmdImpropperArgFmt},
	{"MAIL FROM:", ErrCmdRequiresAddress},
	{"MAIL FROM:<", ErrCmdRequiresAddress},
	{"MAIL FROM:<fred@barney", ErrCmdImpropperArgFmt},
	// alleged 'argument' is all white space
	{"MAIL FROM:      ", ErrCmdRequiresAddress},

	// no space between > and param
	{"MAIL FROM:<fred@barney>SIZE=100", ErrCmdImpropperArgFmt},

	// No arguments
	{"VRFY", ErrCmdRequiresArg},
	{"EXPN", ErrCmdRequiresArg},
	{"AUTH", ErrCmdRequiresArgs},

	// Extra arguments on commands that don't take them.
	{"RSET fred", ErrCmdHasNoArg},
	{"NOOP fred", ErrCmdHasNoArg},
	{"DATA fred", ErrCmdHasNoArg},
	{"QUIT fred", ErrCmdHasNoArg},
}
...
func TestBadParses(t *testing.T) {
	...
	for _, v := range smtpInvalidTests {
		s, err = ParseCmd(v.line)
		if err == nil {
			t.Fatalf("'%s' not detected as error: cmd %v arg '%v'", v.line, s.Cmd, s.Arg)

		}
		if v.err != err {
			t.Fatalf("wrong error on '%s': got '%s' expected '%s'", v.line, err, v.err)
		}
	}
}
...
func TestParam(t *testing.T) {
	s, err := ParseCmd("MAIL FROM:<fred@barney.com> SIZE=1000")
	if err != nil {
		t.Fatal(err)
	}
	...
	s, err = ParseCmd("MAIL FROM:<fred@barney.com>")
	if err != nil {
		t.Fatal(err)
	}
	if len(s.Params) > 0 {
...
250 Okay, I'll believe you for now
503 Out of sequence command
501 Bad: ` + ErrCmdUnrecognized.Error() + `
250 Okay
250 Okay, I'll believe you for now
550 Bad address
250 Okay, I'll believe you for now
